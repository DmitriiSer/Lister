(function () {
    ...
    controllers.controller("ListEditorController", ["$rootScope", "$scope", "$state", "$timeout", "$http", "$ionicScrollDelegate", "server", "browser", "session",
        function ($rootScope, $scope, $state, $timeout, $http, $ionicScrollDelegate, server, browser, session) {
            ...
            var updateToolbarMenuData = function () {
                for (var i = 0; i < toolboxTextIcons.length; i++) {
            ...
            $scope.toolbarMouseenterDelayed = function (item, index) {
                ...
                if (angular.isDefined(item.submenu)) {
                    ...
                    var itemNode = angular.element(document.querySelector("#list-editor-window-side-toolbar"))[0];
                    var itemHeight = parseInt(window.getComputedStyle(itemNode)["width"], 10);
